...
- 提供 13 个面向小说工作流的 Codex Skills：
  ...
  - `webnovel-style-check`：检查反 AI 句式和风格风险，生成只读修改建议报告。
...
v0.9 增加了完整 Write 写作流水线。write 负责准备写作材料、草稿文件和写作指令，不联网，不需要 API，不调用外部模型，也不自动生成完整正文。

- `write`：准备某章写作工作区，生成或保留 `正文/第XXX章.md`，刷新索引、context-pack 和 write-brief，并生成 `章节索引/write-workspace/第XXX章-write-instruction.md`。
- `finalize-write`：用于正文写完后的收尾，生成摘要、更新索引、重建本地检索、运行 deep review 和 doctor。
- `write-status`：查看正文章节、write-brief、context-pack、deep-review、doctor-report 和最近写作指令状态。

Codex 应读取 write-instruction、write-brief 和 context-pack 后再写正文。finalize-write 不会自动修改人物、伏笔、大纲或设定。

## v1.0 新增能力

v1.0 增加了反 AI 句式与风格风险审查。style-check 默认只读，不联网，不使用 API，不使用 embedding provider，也不会自动修改正文。
...
`style-check` 的检测结果需要人工判断。用户可以编辑 `设定集/AI句式禁区.md` 补充项目自定义规则、偏好和例外。该功能适合配合 deep review 使用，但第一版保持独立命令，不会由 review 自动调用。
...
python3 scripts/webnovel.py write-status ../sample-novel-project
```

write 只准备写作材料，不自动生成正文。Codex 应读取 write-instruction、write-brief 和 context-pack 后再写正文。finalize-write 用于写完后的摘要、索引、审查和体检。整个流程不联网、不需要 API。
